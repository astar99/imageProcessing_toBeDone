...
void getPriorInfo(Mat& imgSrc, PriorInfo& priorInfos, const char* name, const int range);
void getResult(Mat& imgSrc, PriorInfo& priorInfo);
void getTemplate(Mat& imgSrc1, Mat& imgSrc2, Mat& imgDst, const char* templateName);
void getPointer(Mat& imgSrc, Mat& imgDst, Mat& imgTemplate);
/**/
void testPerspective(const char* picname, int op);
void test();
/* ksize */
...
void init(){
	...
	priorInfo.x = g_v3fLeastSquaresFitting[0];
	priorInfo.y = g_v3fLeastSquaresFitting[1];
	priorInfo.radiusMax = g_v3fLeastSquaresFitting[2];
	/**
	priorInfo.x = g_v3fDescriptorRoundnessRate[0];
	priorInfo.y = g_v3fDescriptorRoundnessRate[1];
	priorInfo.radiusMax = g_v3fDescriptorRoundnessRate[2];
	/**
...
void initCamera(const char* winname){
	...
	videoCapture.open(0);
	char key = 0;
	createTrackbar("goodMatch", winname, &g_nTrackbarValue6, g_nTrackbarCount6, trackbarCallback6, 0);
	...
	while(1){
		...
		switch(key){
			...
			case '4':
				/* 测试透视正变换 */
				testPerspective("template.jpg", 1);
				break;
			case '5':
				/* 测试透视反变换 */
				testPerspective("templateToSample.jpg", 0);
				break;
			case '6':
				/* 最大值法获取仪表模板 */
				imgSample = imread("./src1_1_1.jpg", 0);
				imgTempate = imread("./src1_1_2.jpg", 0);
				if(!imgSample.data || !imgTempate.data){
					cout << "!!! error !imgSample.data || !imgTempate.data" << endl;
					break;
				}
				imshow("sample", imgSample);
				imshow("template", imgTempate);
				getTemplate(imgSample, imgTempate, imgDst, "src1_1_0.jpg");
				break;
			case '7':
				/* 差影法获取指针 */
				imgSample = imread("./src1_1_2.jpg", 0);
				imgTempate = imread("./src1_1_0.jpg", 0);
				if(!imgSample.data || !imgTempate.data){
					cout << "!!! error !imgSample.data || !imgTempate.data" << endl;
					break;
				}
				imshow("sample", imgSample);
				imshow("template", imgTempate);
				getPointer(imgSample, imgDst, imgTempate);
				break;
		}
	}
}
...
void go() {
	...
	if (!imgSample.data || !imgTempate.data) {
		cout << "!!! warning !imgSample.data || !imgTempate.data" << endl;
		imgSample = imread("./pic/sample0.jpg", IMREAD_GRAYSCALE);
		imgTempate = imread("./pic/template0.jpg", IMREAD_GRAYSCALE);
	}
	imshow("sample", imgSample);
	imshow("template", imgTempate);
...
void getDescriptorArea1(Mat& imgsrc, vector<vector<Point>>& contours, map<int, int>& mapArea){
	...
	for(map<int, int>::iterator it = mapArea.begin(); it != mapArea.end(); it++){
		...
		/* count检测 *
		count++;
		cout << "getDescriptorArea1 n = " << count << "   ";
		if(count % 2 == 0){
			cout << endl;
		}
		waitKey();
	...
	for(map<int, Point>::iterator it = g_mCentroid.begin(); it != g_mCentroid.end(); it++){
		...
		waitKey();
		circle(tempImg, (*it).second, 5, Scalar::all(0));
		/**/
	}
	/**
	cout << "全部质点, press to continue" << endl;
	waitKey();
	...
	for(map<int, Point>::iterator it = g_mCentroid.begin(); it != g_mCentroid.end();){
		...
		if(Ri < g_v3fDescriptorRoundnessRate[2] * 0.85){
	...
	/**
	cout<<"删除靠近圆心的质点， press to continue"<<endl;
	waitKey();
	...
	imshow("getDescriptorArea1", tempImg);
	cout<<"最终拟合的圆，press to continue"<<endl;
	/* 画出最初的圆形目标，对比效果图 */
	waitKey(200);
	...
	circle(tempImg, Point(g_v3fDescriptorRoundnessRate[0], g_v3fDescriptorRoundnessRate[1]), 5, Scalar::all(100));
	imshow("getDescriptorArea1", tempImg);
	/**/
}
/**/
...
void leastSquaresFittingCircle(map<int, Point>& mapcentroid, Vec3f& v3fLeastsquaresfittingcircle){
	/* 将质点先画出来 *
	Mat imgShow = Mat::zeros(g_imgSrc.size(), g_imgSrc.type());
	int n=0;
	for(map<int, Point>::iterator it = mapcentroid.begin(); it != mapcentroid.end(); it++){
		circle(imgShow, it->second, 0, Scalar::all(255));
		waitKey(10);
		imshow("leastSquaresFittingCircle", imgShow);
		/**
		circle(imgShow, it->second, 5, Scalar::all(255));
		n++;
		cout<<"质点 n = "<<n<<endl;
		waitKey();
		circle(imgShow, it->second, 5, Scalar::all(0));
		/**
	}
	/**/
	...
	radius = 0.0f;
	if(mapcentroid.size() < 3){
		cout << "leastSquaresFittingCircle mapcentroid.size() = " << mapcentroid.size() << endl;
		return;
	}
	...
	cout << "center_x = " << center_x
		...
		<< endl;
	/**
	circle(imgShow, Point(center_x, center_y), radius, Scalar::all(255));
	circle(imgShow, Point(center_x, center_y), 1, Scalar::all(255));
	imshow("leastSquaresFittingCircle", imgShow);
	/**
	cout<<"C = "<<C<<"\nD = "<<D<<"\nE = "<<E<<"\nG = "<<G<<"\nH = "<<H<<"\n\na = "<<a<<"\nb = "<<b<<"\nc = "<<c<<endl;
	/**/
...
void getAnnularMask(Mat& mask){
	...
	circle(imgCircleSmall, center, radiusLarge*0.3, scalar);
	...
	for(int i = 0; i < imgsrc.rows; i++){
		for(int j = 0; j < imgsrc.cols; j++){
			if(imgsrc.at<uchar>(i, j)){
				...
				/**/
				Point point(j, i);
				getAngleFromTwoPoints(angle, center, point);
				/* 注意将弧度转化为角度 */
...
void getResult(){
	...
	Point point(g_v3fDescriptorRoundnessRate[0], g_v3fDescriptorRoundnessRate[1]);
	project(g_imgAnnular, mapProj, point, nProjCount);
...
void getDescriptorAndMatch(Mat& imgTemplate, Mat& imgSample){
	...
	*(pDst + step0dst*y + step1dst*x + 1) = *(pSrc + step0src*i + step1src*j + 1);
	*(pDst + step0dst*y + step1dst*x + 2) = *(pSrc + step0src*i + step1src*j + 2);
	/**
	count++;
	}
	}
	...
	/**/
	warpPerspective(imgSample, imgSampleToTemplate, imgHSampleToTemplate, Size(imgTemplate.cols, imgTemplate.rows));
	imshow("imgSampleToTemplate warpPerspective", imgSampleToTemplate);
...
void getPriorInfo(Mat& imgSrc, PriorInfo& priorInfo, const char* name, const int range){
	priorInfo.name = name;
	priorInfo.range = range;
	Mat imgDst = Mat::zeros(imgSrc.size(), imgSrc.type());
	/* blur */
	trackbar0(1, imgSrc, g_imgBlur);
	/* threshold */
	trackbar4(20, g_imgBlur, g_imgThreshod);
	/**/
	getDescriptorRoundnessRate(g_imgThreshod, g_v3fDescriptorRoundnessRate);
	getMask(g_imgThreshod, g_imgMask);
	getRoundTarget(g_imgThreshod, g_imgRound, g_imgMask);
...
void getResult(Mat& imgSrc, PriorInfo& priorInfo){
	/* threshold */
	trackbar4(20, imgSrc, imgSrc);
	getAnnularMask(g_imgMask);
	getAnnularTarget(imgSrc, imgSrc, g_imgMask);
	/* 检查先验信息中的x和y */
	circle(imgSrc, Point(priorInfo.x, priorInfo.y), 10, Scalar::all(255));
	imshow("getResult", imgSrc);
	Mat imgShow = Mat::zeros(imgSrc.size(), imgSrc.type());
	/* 对应mapProj 的角度index，投影值 */
	map<int, int> mapProj;
	Vec2i projPointer;
	int nCountPerDegree = 2;
	int nProjCount = 360 * nCountPerDegree;
	MatND userData = Mat(nProjCount, 1, CV_32F);
	/* 将imgSrc 向中心投影，输出mapProj */
	Point point(priorInfo.x, priorInfo.y);
	project(imgSrc, mapProj, point, nProjCount);
	/* 使用mapProj 中的数据画直方图 */
	for(int i = 0; i < nProjCount; i++){
		userData.at<float>(i) = mapProj[i];
	}
	...
	cout << "getResult projPointer: " << projPointer << endl;
	/* 使用 priorInfo 中的先验信息求指针读数 */
	float rangePointer = 0, rangeFull = 0;
	float scalePointer = 0, result = 0;
	/* 将 index 转换为对应的角度值 */
	scalePointer = 1.0 * projPointer[0]/nCountPerDegree;
	rangePointer = scalePointer - priorInfo.scaleStart;
	if(rangePointer < 0){
		rangePointer += 360;
	}
	rangeFull = priorInfo.scaleEnd - priorInfo.scaleStart;
	if(rangeFull < 0){
		rangeFull += 360;
	}
	result = priorInfo.range * rangePointer/rangeFull;
	cout<<"getResult result = "<<result<<endl;
}
void getTemplate(Mat& imgSrc1, Mat& imgSrc2, Mat& imgDst, const char* templateName){
	Mat imgTemp = Mat::zeros(imgSrc1.size(), imgSrc1.type());
	/**
	uchar* pDataSrc1 = imgSrc1.data;
	uchar* pDataSrc2 = imgSrc2.data;
	uchar* pDataDst = imgTemp.data;
	/**/
	uchar* pDataSrc1, *pDataSrc2, *pDataDst ;
	int step0 = imgSrc1.step[0], step1 = imgSrc1.step[1];
	/**/
	for(int i = 0; i < imgSrc1.rows; i++){
		pDataSrc1 = imgSrc1.ptr<uchar>(i);
		pDataSrc2 = imgSrc2.ptr<uchar>(i);
		pDataDst = imgTemp.ptr<uchar>(i);
		for(int j = 0; j < imgSrc1.cols; j++){
			if(imgSrc1.step[1] < 3){
				pDataDst[j] = pDataSrc1[j] > pDataSrc2[j]? pDataSrc1[j]:pDataSrc2[j];
			}
			else{
				pDataDst[j*step1+0] = pDataSrc1[j*step1 + 0] > pDataSrc2[j*step1 + 0] ? pDataSrc1[j*step1 + 0] : pDataSrc2[j*step1 + 0];
				pDataDst[j*step1 + 1] = pDataSrc1[j*step1 + 1] > pDataSrc2[j*step1 + 1] ? pDataSrc1[j*step1 + 1] : pDataSrc2[j*step1 + 1];
				pDataDst[j*step1 + 2] = pDataSrc1[j*step1 + 2] > pDataSrc2[j*step1 + 2] ? pDataSrc1[j*step1 + 2] : pDataSrc2[j*step1 + 2];
			}
		}
	}
	imgDst = imgTemp;
	imshow("getTemplate", imgDst);
	imwrite(templateName, imgDst);
	cout<<"getTemplate"<<endl;
}
void getPointer(Mat& imgSrc, Mat& imgDst, Mat& imgTemplate){
	Mat imgTemp = Mat::zeros(imgSrc.size(), imgSrc.type());
	imgTemp = imgTemplate - imgSrc;
	imgDst = imgTemp;
	imshow("getPointer", imgDst);
	imwrite("src1_3_1.jpg", imgDst);
	cout<<"getPointer"<<endl;
}
/**/
void testPerspective(const char* picname, int op){
	Mat img = imread(picname, 0);
	if(!img.data && op){
		img = imread("sec1_1.jpg", 0);
	}
	else if(!img.data && !op){
		img = imread("templateToSample.jpg", 0);
	}
	...
	if(op){
		...
		transform = getPerspectiveTransform(corners_trans, corners);
	}
	if(!transform.data){
		cout << "!!! error transform.data" << endl;
		return;
	}
	/**/
	...
	Mat img_trans = Mat::zeros(img_height, img_width, CV_8U);
	int count = 0;
	for(int i = 0; i<img_height; i++){
	...
	/**/
	if(op){
		imwrite("templateToSample.jpg", img_trans);
	}
	else{
		imwrite("sampleToTemplate.jpg", img_trans);
	}
	/**/
	warpPerspective(img, img_trans, transform, img.size());
	imshow("testWarpPerspective", img_trans);
	/**/
}
void test(){
	...
	/**
	leastSquaresFitting(mapCentroid, v3fLeastSquaresFitting);
	/* 画出拟合圆 */
...
void testDrawHistogramFromUserData(){
	...
	Point point(g_v3fLeastSquaresFitting[0], g_v3fLeastSquaresFitting[1]);
	project(imgCentroid, mapProj, point, 360);
	waitKey(500);
